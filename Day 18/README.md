...
1. Create `disk\_check.sh` with:

   * A function `check\_disk` that checks disk usage of `/` using `df -h`
   * A function `check\_memory` that checks free memory using `free -h`
...
1. Create `strict\_demo.sh` with `set -euo pipefail` at the top
...
Create `system\_info.sh` that uses functions for everything:
...
* Function syntax: `function\_name() { ... }`
* Local vars: `local MY\_VAR="value"`
